...
#1 original csv to clean csv
def medicine_csv_clean():
...
#2 from clean csv file to url
def medicine_csv_to_url():
...
#3 from clean csv file to item dictionary
def medicine_csv_to_item():
    with open('../data/medicine.csv', 'rb') as infile, open('../data/item.dic', 'wb') as outfile:
        spamreader = csv.reader(infile, delimiter=',')
        for row in spamreader:
            outfile.write(row[0] + "\n")

#4 count valid url from baike.com
def count_valid_url_in_baike():
...
#8 create synonym in mysql
def create_synonym_in_mysql():
...
#9 tf-idf dictionary to normal dictionary
def dictionary_tfidf_to_normal():
...
#10 create dic from 15 file
def get_value_from_15_attributes():
    db = MySQLdb.connect(host='localhost', user='root', passwd='123456', db='graduate', charset='utf8')
    cur = db.cursor()
...
#13 see different between two item file
def diff_items():
